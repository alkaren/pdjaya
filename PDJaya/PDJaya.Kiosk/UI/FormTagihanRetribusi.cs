...
{
    public partial class FormTagihanRetribusi : Form
    {
        //declare item layout
        ...
        {
            DataTagihan =  await RetribusiManager.GetBills();
            if (DataTagihan == null)
            {
        ...
        void Configure()
        {
            ...
            picBox.Image = Properties.Resources.layar_retribusi;
        ...
        async void timer_Tick(object sender, EventArgs e)
        {
            ...
            if (info.StatusCode == GlobalVars.SUCCESS_CODE)
            {
                ...
                if (!string.IsNullOrEmpty(cardno) && IsReadyToPay)
                {
                    ...
                    var payment = new Payment() { Amount=Tagihan.Amount, CardNo=cardno, MarketNo=GlobalVars.CurrentTenant.MarketNo, PaymentDate=DateTime.Now, Status= PaymentStatus.Success, StoreNo=GlobalVars.CurrentTenant.StoreNo, TransactionCode="1", TransactionID=Tagihan.BillID, TransactionName=$"Retribusi - {Tagihan.Month} - {Tagihan.Year}" };
                    ...
                    {
                        var res = await RetribusiManager.PushPayment(new List<Payment> { payment });
                        if (res)
                        {
                            MethodInvoker methodInvokerDelegate = delegate ()
                            {
                                var newFrm = new FormRetribusiBerhasil(payment);
                                newFrm.Show();
                                this.Close();
                            };
        ...
        {
            var newFrm = new FormRetribusiGagal(info,Message);
            newFrm.Show();
            this.Close();
        }
